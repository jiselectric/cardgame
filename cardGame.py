...
class CardBatchMaker:
    ...
    def makeGame(self):
        ...
        while len(solver.answers) < self.count:
            ...
            solver = Solver(self.batches, target)
        return target, solver.answers

class Host:
    def __init__(self, cards, target, answers, lives):
        self.point = 0 
        self.count = 0
        self.cards = cards
        self.target = target
                    ...
                    for op in ('+', '-', '*', '/'):
                        ...
                        if result == target:
                            if op == '+' or op == '*':
                                ...
                                m = i
                                n = j
                            ans = batches[i] + batches[op] + batches[j]
...
import time
cbm = CardBatchMaker()
cards, target, answers = cbm.makeGame()
host = Host(cards, target, answers)
host.getTables()
host.showValues()
time.sleep(5)
